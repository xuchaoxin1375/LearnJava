    ...
    void divide(double dividend ,double divisor) throws OddException{
        ...
        if(divisor%2!=0){
            try {
                ...
                /*this statement will invoke the method in the Odd Exception Class :the getMessage()  method */
                e.printStackTrace();
            }
        }
        /*when there is no exception ,then do the division ,meanwhile Print the result .*/
        else
            System.out.println(String.format("%.2f/%.2f=", dividend ,divisor)+new DecimalFormat("0.00").format(dividend /divisor ));
